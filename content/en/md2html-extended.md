...
## Index
### Front Page
This file MUST exist in the folder of each TS document. It contains:

```md
title: Name of Specification
status: "Draft"
    * Default value until ratified by the OMA Board
version: "Version x.y"
date: "DD MMM YYYY"
    * Omit unless the document has status Candidate or final Approval
documentName: "OMA-TS-FunctionalName-Vx_y_z"
files: the list of markdown files that the document consists of in order
```
...
>Note: Depending on the line thickness in figures, upon the SVG scaling factor and the browser zoom setting some lines may not be displayed when viewed from GitHub. This can be mitigated by increasing the browser zoom setting.
...
* Ensure contiguous text in a line is in a single text box and not multiple text boxes. If multiple text boxes are used then when the figures are converted to PDF the figures may appear incorrect.
